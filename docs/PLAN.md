...
| ✅ **P0.A-5** | `pnpm check` 聚合命令 + Git hooks            | `docs/specs/P0.A-5-check-and-hooks.md`     | P0.A-3, P0.A-4 | `package.json` 新增 `check` 脚本（format:check→lint→typecheck→test）+ `postinstall` + `simple-git-hooks` + `lint-staged` 两块顶层 config | `pnpm check` 四段全绿；pre-commit 真实拦截 `any`；pre-commit 自动 fix 缺空格/缺分号后 commit 成功         | 低   |
